...
module.exports = (robot) ->
  ...
  robot.respond /おる|きた/, (msg) ->
    ...
    for mac, user of orca.members
      if user == name
        ...
        robot.brain.set "orca", orca
        robot.brain.save()
    msg.send msg.random ['今日も元気にがんばろー！', 'はいはーい。いらっしゃいませー']

  robot.respond /帰る|退社|かえる/, (msg) ->
    name = msg.envelope.user?.name
    orca = robot.brain.get "orca"
    for mac, user of orca.members
      if user == name
        for i,m of orca.macs
          if m == mac
            ...
            robot.brain.set "orca", orca
            robot.brain.save()
    msg.send msg.random ['えー、帰っちゃうのー？', 'おつかれさまでしたー']
  ...
  robot.hear /お(る|ら)ん?(よ|か|や)/, (msg) ->
    ...
    #   if user = orca.members[mac] && ! (mac in orca.macs)
